...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        print("#" * 80)
        prompt_backup = input('the database has a back up? [y/N]: ')
        if prompt_backup.lower() == 'y':
            users = User.objects.all()
            print("Found %s users!" % users.count())
            prompt_show_all_users = input('want to list all these users? [y/N]: ')
            ...
            prompt_show_non_scielo_users = input('want to list all these users? [y/N]: ')
            ...
            if has_special_users:
                prompt_to_exclude_special_users = input(
            ...
            print("#" * 80)
            prompt_confirm_modify = input('Are you sure? the process CAN NOT BE UNDONE [y/N]: ')

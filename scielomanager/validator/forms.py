    ...
    def clean_file(self):
        ...
        if _file:
            if _file.content_type not in ['text/xml', 'application/xml', ]:
                raise forms.ValidationError(_("This type of file is not allowed! Please select another file."))

            if _file.size > settings.VALIDATOR_MAX_UPLOAD_SIZE:
                raise forms.ValidationError(_("The file's size is too large! Please select a smaller file."))

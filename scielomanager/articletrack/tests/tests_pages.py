...
class CheckinListTests(WebTest):
    ...
    def _makeOne(self):
        self.checkin = modelfactories.CheckinFactory.create()

        #Get only the first collection and set to the user
        collection = self.checkin.journals.all()[0].collection
    ...
    def test_checkin_list_display_detail_and_history_button(self):
        ...
        response.mustcontain('<a href="/arttrack/package/1/" class="btn btn-mini btn" type="button">History</a>')
    ...
    def test_status_code_package_history(self):
        ...
        response = self.app.get(reverse('checkin_history',
            args=[self.checkin.pk]), user=self.user)
...
class NoticeListTests(WebTest):
    ...
    def _makeOne(self):
        self.notice = modelfactories.NoticeFactory.create()

        #Get only the first collection and set to the user
        collection = self.notice.checkin.journals.all()[0].collection
        collection.add_user(self.user, is_manager=True, is_default=True)
    ...
    def test_status_code_notice_list(self):
        ...
        response = self.app.get(reverse('notice_detail',
            args=[self.notice.checkin.pk]), user=self.user)
    ...
    def test_status_code_notice_list_without_waflle_flag(self):
        ...
        response = self.app.get(reverse('notice_detail',
            args=[self.notice.checkin.pk]), user=self.user, expect_errors=True)
    ...
    def test_notice_list_must_have_link_to_package_history(self):
        ...
        response.mustcontain('<a href="/arttrack/package/1/">Package history</a>')

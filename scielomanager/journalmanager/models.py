...
class LinkBetweenArticles(models.Model):
    ...
    def __repr__(self):
        return u'<%s referrer="%s" link_to="%s" link_type="%s">' % (
                self.__class__.__name__, self.referrer, self.link_to,
                self.link_type)
...
class Article(models.Model):
    ...
    links_to_related_articles_are_pending = models.BooleanField(default=False)
    ...
    related_articles = models.ManyToManyField('self', through='LinkBetweenArticles',
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk:
            ...
            if self.article_type in ['correction', 'article-commentary']:
                self.links_to_related_articles_are_pending = True
    ...
    #def add_link_to_article(self, target, link_type):
    #    """ Cria relação com outra entidade tipo `Article`.
    #    """
    #    LinkBetweenArticles.objects.create(
    #            referrer=self, link_to=target, link_type=link_type)

    def __repr__(self):
        return u'<%s aid="%s" domain_key="%s">' % (self.__class__.__name__,
                self.aid, self.domain_key)

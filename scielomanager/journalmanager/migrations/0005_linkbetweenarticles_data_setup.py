...
def get_value(xml, expression):
    """ Busca `expression` em `xml` e retorna o resultado da primeira ocorrência.

    Função copiada de `journalmanager.models.Article.get_value`.
    ...
    """
    try:
        first_occ = xml.xpath(expression)[0]
    except IndexError:
        return None
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for article in orm.Article.objects.only('xml').filter(doi=u''):
            ...
            if article.article_type in ['correction', 'article-commentary']:
                article.links_to_related_articles_are_pending = True
            else:
    ...
    models = {
        ...
        'journalmanager.article': {
            ...
            'doi': ('django.db.models.fields.CharField', [], {'max_length': '2048', 'db_index': 'True', 'default': "''"}),

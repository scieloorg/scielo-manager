...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        try:
            use_license_code_nd = orm.UseLicense.objects.get(license_code='nd')
        except orm.UseLicense.DoesNotExist:
            pass # Data already migrated
        else:
            for journal in orm.Journal.objects.filter(use_license__license_code=''):
                journal.use_license = use_license_code_nd
                journal.save()

            for issue in orm.Issue.objects.filter(use_license__license_code=''):
                issue.use_license = use_license_code_nd
                issue.save()
